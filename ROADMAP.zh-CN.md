...
_最近更新：2026-05-19_ · [English](ROADMAP.md)
...
- Python：`1.4.0`
- TypeScript：`1.4.0`
- 1.x 期间冻结 21 个公共 MCP 工具（CI 强制检查）。
...
每个 minor 版本聚焦一个数据域。跨源融合工具与其数据依赖一同发布。

### 1.5.0 — 关卡数据域 + 跨源融合

**主：关卡数据域**
- `list_stages(chapter?, type?)` — 关卡列表（主线 / 活动）。
- `get_stage_info(stage_id)` — 关卡详情：地图、波次、敌人配置。
- `search_stages(pattern)` — 关卡名称和标签的正则搜索。
...
1.x 工具面持续增长（1.4.0 已 21 个，预计 1.8.0 后达 30+）。旗舰 长上下文模型不在乎；但对 128K 级别模型，每个工具 schema 都吃 prompt 预算并降低工具选择准确率。
...
**方法**：服务端按 *schema 形态* 合并，而非按数据域合并。合并参数 结构和输出形态相似的工具，保留语义真正不同的工具。预估缩减： 21 → ~14 个工具（约 1/3），不损失能力。
...
4. **跨源融合工具绑定其数据依赖**——`get_stage_enemies` 与关卡一同
   发布，不会提前。
